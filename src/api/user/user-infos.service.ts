...
export class UsersInfosService {
  constructor(private readonly usersService: UserService, private readonly userRepository: UsersRepository) {}
  ...
  public async getInfo(user: User, phone: string, infoLevel: UserInfoLevel): Promise<UserInfo | string> {
    const targetUser = await this.userRepository.getUserFromPhone(phone);
    if (targetUser === null) {
      return phone;
    }
